...
class PDFObject
{
        ...
        foreach ($textCleaned as $line) {
            ...
            if (preg_match('/BT$/', $line)) {
                ...
                if ('q' == $line[-1] || 'Q' == $line[-1]) {
                    ...
                    $sections[] = $line;
                } else if (preg_match('/(?<!\w)B[DM]C$/', $line)) {
                    // Begin marked content sequence
                    $sections[] = $line;
                } else if (preg_match('/(?<!\w)[DM]P$/', $line)) {
                    // Marked content point
                    $sections[] = $line;
                } else if (preg_match('/(?<!\w)EMC$/', $line)) {
                    // End marked content sequence
                    $sections[] = $line;
                } else if (preg_match('/(?<!\w)cm$/', $line)) {
                    // Graphics position change commands
                    $sections[] = $line;
                } else if (preg_match('/(?<!\w)Tf$/', $line)) {
                    // Font change commands
                    $sections[] = $line;
                } else if (preg_match('/(?<!\w)Do$/', $line)) {
                    ...
                    case 'BDC':
                        if (preg_match('/(<<.*>>)$/', $command[self::COMMAND], $match)) {
                            ...
                            if (isset($dict['ActualText']) && \is_string($dict['ActualText']) && '' !== $dict['ActualText']) {
                                ...
                                } else if ('<' == $dict['ActualText'][0] || '(' == $dict['ActualText'][0]) {
                                    // Simulate a 'Tj' command on the stack
                                    $marked_stack[] = [
                                        'ActualText' => $this->getCommandsText($dict['ActualText'].'Tj')[0],
                                    ];
    ...
    public function getCommandsText(string $text_part, int &$offset = 0): array
    {
        ...
        if ('TJ' == $operator) {
            ...
            $command = $subcommand;
        } else if ('Tj' == $operator || "'" == $operator || '"' == $operator) {
