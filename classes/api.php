...
class local_haccgen_api {
     ...
     * @return string
     */
    private static function moodle_curl_error_message(\curl $curl): string {
    ...
    public static function generate_content_for_topics(array $topics, $data) {
        ...
        if (isset($response['results']) && is_array($response['results'])) {
            ...
            foreach ($response['content'] as $c) {
                ...
                // Backward/alternate shape: sections may be directly under result.
                if (
                        empty($generated['content_sections']) &&
                        !empty($res['content_sections']) &&
                        is_array($res['content_sections'])
                    ) {
                    $generated['content_sections'] = $res['content_sections'];
                }
                // Newer service shape: subtopics can be returned directly.
                if (empty($generated['content_sections']) && !empty($res['subtopics']) && is_array($res['subtopics'])) {
                    $generated['content_sections'] = array_map(static function ($sub) {
                        if (!is_array($sub)) {
                            return [
                        ...
                        return [
                            'section_title' => $sub['title'] ?? 'Untitled Section',
                            'content' => $sub['content']
                                ?? ($sub['description'] ?? ''),
                            'examples' => is_array($sub['examples'] ?? null) ? $sub['examples'] : [],
                        ];
                    }, $res['subtopics']);
                }
                if (
                        empty($generated['topic_learning_objectives']) &&
                        !empty($res['topic_learning_objectives']) &&
                        is_array($res['topic_learning_objectives'])
                    ) {
                    $generated['topic_learning_objectives'] = $res['topic_learning_objectives'];
                }
                if (
                        empty($generated['topic_learning_objectives']) &&
                        !empty($c['topic_learning_objectives']) &&
                        is_array($c['topic_learning_objectives'])
                    ) {
                    $generated['topic_learning_objectives'] = $c['topic_learning_objectives'];
                }

                $quizdata = $res['quiz_data'] ?? ($c['quiz_data'] ?? null);
                // Alternate shape: questions can be emitted directly under result.
                if (
                    (empty($quizdata) || !is_array($quizdata)) &&
                    !empty($res['questions']) &&
                    is_array($res['questions'])
                ) {
                    $quizdata = [
                        'quiz_title' => $res['quiz_title'] ?? ($c['topic_title'] ?? 'Quiz'),
    ...
    private static function post_json_via_curl(string $url, array $data) {
        ...
        if ($response === false) {
            throw new \moodle_exception('apierror', 'local_haccgen', '', 'HTTP request failed.');
        }

        $errno = $curl->get_errno();
        $errstr = self::moodle_curl_error_message($curl);
        if ($errno) {
            ...
            $message = 'cURL error ' . $errno . ': ' .
                ($errstr !== '' ? $errstr : '(no message)');

            throw new \moodle_exception('apierror', 'local_haccgen', '', $message);
        }

        if ($status < 200 || $status >= 300) {
            $raw = (string) $response;
            $snip = mb_substr(preg_replace('/\s+/', ' ', strip_tags($raw)), 0, 220);
            self::capture_api_metrics([
                'url' => $url,
                'action' => $data['action'] ?? null,
                'payload' => $jsonpayload,
            ...
            if ($snip !== '') {
                $msg .= ' — response preview: ' . $snip;
            }
            throw new \moodle_exception('apierror', 'local_haccgen', '', $msg);
        }
        ...
        if (json_last_error() !== JSON_ERROR_NONE || !is_array($decoded)) {
            ...
            $msg = 'Invalid JSON from API (JSON error: ' . json_last_error_msg() . ')';
            if ($snip !== '') {
                $msg .= ' — preview: ' . $snip;
            }
            throw new \moodle_exception('apierror', 'local_haccgen', '', $msg);
    ...
    private static function post_json_via_curl_content(string $url, array $data) {
        ...
        // Redact sensitive values before writing payloads to logs.
        $redactpayload = function ($value) use (&$redactpayload) {
            if (!is_array($value)) {
                return $value;
            }
            $sensitivekeys = [
                'api_key', 'api_secret', 'token', 'access_token', 'refresh_token',
                'authorization', 'x-internal-secret',
            ];
            $out = [];
            foreach ($value as $k => $v) {
        ...
        // Build a compact JSON-shape summary that is safe to log.
        $payloadshape = function (array $payload) use ($redactpayload): string {
            $shape = [
                'action' => $payload['action'] ?? null,
                'request_id' => $payload['request_id'] ?? null,
                'top_level_keys' => array_keys($payload),
                'subtopics_count' => isset($payload['subtopics']) && is_array($payload['subtopics'])
                    ? count($payload['subtopics'])
                    : 0,
                'include_quiz_true_count' => 0,
                'quiz_count_total' => 0,
                'course_data_keys' => isset($payload['course_data']) && is_array($payload['course_data'])
                    ? array_keys($payload['course_data'])
                    : [],
            ];

            if (!empty($payload['subtopics']) && is_array($payload['subtopics'])) {
                foreach ($payload['subtopics'] as $subtopic) {
                    if (!empty($subtopic['include_quiz'])) {
                        $shape['include_quiz_true_count']++;
                    }
                    $shape['quiz_count_total'] += (int) ($subtopic['quiz_count'] ?? 0);
                }
            }

            // Keep a small redacted preview of the actual JSON body shape.
            $shape['payload_preview'] = $redactpayload($payload);
            $encoded = json_encode($shape, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
            if (!is_string($encoded)) {
                return '{"error":"unable_to_encode_payload_shape"}';
            }
        ...
        $dopost = function (string $endpoint, array $payload) use ($log, $payloadshape) {
            ...
            $log("POST payload shape: " . $payloadshape($payload));
            ...
            $start = microtime(true);
            $response = $curl->post($endpoint, $jsonpayload, $options);
            $elapsedms = (int) round((microtime(true) - $start) * 1000);
        ...
        $dogetjson = function (string $downloadurl) use ($log) {
            ...
            $start = microtime(true);
            $body = $curl->get($downloadurl, null, $options);
            $elapsedms = (int) round((microtime(true) - $start) * 1000);
            $info = $curl->get_info();
            $status = $info['http_code'] ?? 0;

            if ($body === false) {
                throw new \moodle_exception('apierror', 'local_haccgen', '', 'Download failed.');
            }
