...
// Draft itemid for editor uploads (Tiny/uploader needs a stable draft itemid).
$contenteditordraftitemid = (int) (\local_haccgen\session_store::get('contenteditordraftitemid') ?? 0);
if ($contenteditordraftitemid <= 0) {
    $contenteditordraftitemid = file_get_unused_draft_itemid();
    \local_haccgen\session_store::set('contenteditordraftitemid', $contenteditordraftitemid);
}

// Make sure the draft area is initialised for this user+itemid (safe no-op if already exists).
$userctx = context_user::instance($USER->id);
file_prepare_draft_area(
    $contenteditordraftitemid,
    $userctx->id,
    'user',
    'draft',
    $contenteditordraftitemid,
...
$haccgendata = \local_haccgen\session_store::get('haccgen_data');
...
if (!empty($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($postedlang !== '') {
        $haccgendata->activelang = $postedlang;
        \local_haccgen\session_store::set('haccgen_data', $haccgendata);
    }

    // Always use session value from here on (never trust empty POST).
    $activelang = $haccgendata->activelang ?? 'English';
    ...
    if ($step == 1) {
        ...
        $validsummary = ['yes', 'no'];  // ADD THIS LINE.
        $mintopics = 2;
        $maxtopics = 10;
        ...
        $data->coursesummary = required_param('coursesummary', PARAM_ALPHA);
        if (!in_array($data->coursesummary, $validsummary)) {
            $errors['coursesummary'] = get_string('please_select', 'local_haccgen');
        }
        ...
        $data->targetaudience = $haccgendata->targetaudience ?? '';
        $data->description = $haccgendata->description ?? '';
        $data->generationtype = $haccgendata->generationtype ?? 'ai';
        $data->pdf_file = $haccgendata->pdf_file ?? '';
        $data->pdf_fileid = $haccgendata->pdf_fileid ?? 0;
        $data->pdf_reference_url = $haccgendata->pdf_reference_url ?? '';
        $data->customprompt = $haccgendata->customprompt ?? '';
        $data->activelang = $activelang;
    } else if ($step == 3) {
        ...
        $data->targetaudience = $haccgendata->targetaudience ?? '';
        $data->description = $haccgendata->description ?? '';
        $data->levelofunderstanding = $haccgendata->levelofunderstanding ?? 'Beginner';
        $data->toneofnarrative = $haccgendata->toneofnarrative ?? 'Formal';
        $data->courseduration = $haccgendata->courseduration ?? 'Less than 15 minutes';
        $data->generationtype = $haccgendata->generationtype ?? 'ai';
        $data->pdf_file = $haccgendata->pdf_file ?? '';
        $data->pdf_fileid = $haccgendata->pdf_fileid ?? 0;
        $data->customprompt = $haccgendata->customprompt ?? '';
    ...
    if (empty($errors)) {
        $haccgendata->TOPICTITLE = $data->coursename ?? '';
        $haccgendata->targetaudience = $data->targetaudience ?? '';
        $haccgendata->description = $data->description ?? '';
        $haccgendata->levelofunderstanding = $data->levelofunderstanding ?? '';
        $haccgendata->toneofnarrative = $data->toneofnarrative ?? '';
        $haccgendata->courseduration = $data->courseduration ?? '';
        $haccgendata->generationtype = $data->generationtype ?? 'ai';
        $haccgendata->pdf_file = $data->pdf_file ?? '';
        $haccgendata->pdf_fileid = $data->pdf_fileid ?? 0;
        $haccgendata->courselanguage = $data->courselanguage ?? 'English';
        $haccgendata->numberoftopics = $data->numberoftopics ?? 5;
        ...
        if ($step == 3 && $action === 'save' && !empty($data->topic_order)) {
                ...
                $job = (object) [
                    ...
                    'inputjson' => json_encode(
                        [
                            'topics' => $topics,
                            'options' => [
                                'coursename' => $haccgendata->TOPICTITLE ?? '',
                                'targetaudience' => $haccgendata->targetaudience ?? '',
                                'description' => $haccgendata->description ?? '',
                                'levelofunderstanding' => $haccgendata->levelofunderstanding ?? 'Beginner',
            ...
            if ($isdraft) {
                ...
                if ($haspayload) {
                    ...
                    $haccgendata->topicsjson = $topicsflat;
                    $haccgendata->quizjson = $quizmap;
                    $haccgendata->canonical_payload_json = $payloadjson;
                    $haccgendata->canonical_payload = $payloadarr;
                    \local_haccgen\session_store::set('haccgen_data', $haccgendata);

                    $log('DRAFT_BRANCH_FROM_PAYLOAD', ['topics_count' => count($topicsflat), 'quizzes_count' => count($quizmap)]);
                } else {
                    if ($topicsjsonreq !== '') {
                        $haccgendata->topicsjson = json_decode($topicsjsonreq, true) ?? [];
                    }
                    if ($quizjsonreq !== '') {
                        $haccgendata->quizjson = json_decode($quizjsonreq, true) ?? [];
                    }
                    $haccgendata->canonical_payload_json = '';
                    $haccgendata->canonical_payload = null;

                    $log('DRAFT_BRANCH_FROM_LEGACY', [
                        'topics_count' => count($haccgendata->topicsjson ?? []),
                        'quizzes_count' => count($haccgendata->quizjson ?? []),
                    ]);
                }

                \core\session\manager::write_close();
                redirect(new moodle_url('/local/haccgen/savedraft.php', [
                    'id' => $courseid,
                    'step' => $step,
                    'sesskey' => sesskey(),
                ]));
            ...
            if ($haspayload) {
                ...
                // Insert content snapshot and link timestamp to it.
                $createdbatchid = uniqid('created_', true);
                $contentrecord = new stdClass();
                $contentrecord->courseid = $courseid;
                $contentrecord->userid = (int) $USER->id;
                $contentrecord->batchid = $createdbatchid;
                $contentrecord->status = 'published';
                $contentrecord->topicsjson = json_encode($topics, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
                $contentrecord->quizjson = json_encode($quizcontents, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
                $contentrecord->timecreated = time();
                $contentrecord->timemodified = $contentrecord->timecreated;
                $contentrecord->id = $DB->insert_record('local_haccgen_content', $contentrecord);
                ...
                \local_haccgen\session_store::delete('haccgen_data');
            ...
            $basetopics = json_decode(json_encode($haccgendata->topics ?? []), true);
...
$formdata = [
    ...
    'targetaudience' => $haccgendata->targetaudience ?? '',
    'audiencetags' => !empty($haccgendata->targetaudience)
        ? explode(',', $haccgendata->targetaudience)
        : [],

    'language' => isset($_POST['language'])
        ? clean_param($_POST['language'], PARAM_TEXT)
        : ($haccgendata->language ?? 'english'),

    'levelofunderstanding' => isset($_POST['levelofunderstanding'])
        ? clean_param($_POST['levelofunderstanding'], PARAM_TEXT)
        : ($haccgendata->levelofunderstanding ?? ''),

    'toneofnarrative' => isset($_POST['toneofnarrative'])
        ? clean_param($_POST['toneofnarrative'], PARAM_TEXT)
        : ($haccgendata->toneofnarrative ?? ''),

    'courseduration' => isset($_POST['courseduration'])
        ? clean_param($_POST['courseduration'], PARAM_TEXT)
        : ($haccgendata->courseduration ?? ''),
    'coursesummary' => isset($_POST['coursesummary'])
        ? clean_param($_POST['coursesummary'], PARAM_ALPHA)
        : ($haccgendata->coursesummary ?? 'no'),
    'description' => $haccgendata->description ?? '',
    'generationtype' => $haccgendata->generationtype ?? 'ai',
    'pdfuploaded' => $haccgendata->pdf_file ?? '',
    'currentstep' => $step,
    'stepstates' => $stepstates,
    'has_levelofunderstanding' => !empty($haccgendata->levelofunderstanding),
    'has_toneofnarrative' => !empty($haccgendata->toneofnarrative),
    'has_courseduration' => !empty($haccgendata->courseduration),
    'is_level_beginner' => ($haccgendata->levelofunderstanding ?? '') === 'Beginner',
    'is_level_intermediate' => ($haccgendata->levelofunderstanding ?? '') === 'Intermediate',
    'is_level_advanced' => ($haccgendata->levelofunderstanding ?? '') === 'Advanced',
    'is_tone_formal' => ($haccgendata->toneofnarrative ?? '') === 'Formal',
    'is_tone_conversational' => ($haccgendata->toneofnarrative ?? '') === 'Conversational',
    'is_tone_engaging' => ($haccgendata->toneofnarrative ?? '') === 'Engaging',
    ...
    'numberoftopics' => $haccgendata->numberoftopics ?? 5,
    'is_language_en' => ($haccgendata->courselanguage ?? '') === 'English',
    'is_language_hn' => ($haccgendata->courselanguage ?? '') === 'Hindi',
    'customprompt' => $haccgendata->customprompt ?? '',
    'contenteditor' => $textarea,
    'contenteditor_itemid'      => $contenteditordraftitemid,
    'sesskey'                   => sesskey(),
...
if ($step == 3) {
    ...
    $data->targetaudience = $haccgendata->targetaudience ?? '';
    $data->levelofunderstanding = $haccgendata->levelofunderstanding ?? 'Beginner';
    $data->toneofnarrative = $haccgendata->toneofnarrative ?? 'Conversational';
    $data->courseduration = $haccgendata->courseduration ?? 'Less than 30 minutes';
    $data->generationtype = $haccgendata->generationtype ?? 'ai';
    $data->pdf_fileid = $haccgendata->pdf_fileid ?? 0;
    $data->description = $haccgendata->description ?? '';
    $data->customprompt = $haccgendata->customprompt ?? '';
    ...
    if (empty($formdata['errors'])) {
        try {
            ...
            $haccgendata->learning_objectives1 = $learningobjectives1;

            $haccgendata->case_study_data = is_array($casestudy)
                ? $casestudy
            ...
            // Store timing for display in step 3 template.
            $haccgendata->topic_generation_duration_seconds = $topicgenerationduration;
            $haccgendata->topic_generated_at = time();
            ...
            // Prefer the real detail: generic code uses errorcode "error" with text in $a; getMessage() needs lang string "error".
    ...
    // Pass topic generation timing to template (from current run or previous session).
    if (!empty($formdata['has_topics']) && isset($haccgendata->topic_generation_duration_seconds)) {
        $secs = (float) $haccgendata->topic_generation_duration_seconds;
        $formdata['topic_generation_duration_seconds'] = $secs;
        $formdata['topic_generation_duration_text'] = ($secs >= 60)
        ...
        $formdata['topic_generated_at_formatted'] = userdate(
            $haccgendata->topic_generated_at ?? time(),
            get_string('strftimedatetime', 'langconfig')
        );
        $formdata['topic_generated_in_text'] = get_string('topic_generated_in', 'local_haccgen', (object) [
            'duration' => $formdata['topic_generation_duration_text'],
            'time' => $formdata['topic_generated_at_formatted'],
        ]);
    }
} else if ($step == 4) {
    ...
    if ($justgenerated) {
        ...
        if ($recentautosave && !get_user_preferences('haccgen_autosave_notified_' . $courseid, false)) {
            \core\notification::success(get_string('autosavedsuccess', 'local_haccgen'));
    ...
    foreach ($formdata['topics'] as $topicindex => $topic) {
        ...
        if (!empty($quizdata1['quiz_title']) && !empty($quizdata1['questions'])) {
            ...
            $quizlist[] = ['id' => "topic{$topicindex}_quiz", 'title' => $quiztitle];
            $quizindex++;
        }
    }
    // Calculate totals.
    ...
    $formdata['scorm_meta'] = $haccgendata->scorm_meta;
    if (!empty($haccgendata->scorm_meta)) {
        $meta = $haccgendata->scorm_meta;
        $formdata['total_slides'] = $meta['total_slides'] ?? 0;
        $formdata['totalquizzes'] = $meta['totalquizzes'] ?? 0;
    ...
    // HAMBURGER MENU DATA (CRITICAL - MUST BE AFTER TOPICS ARE FULLY PROCESSED).
    ...
    // Pass content maps for editor.
    $formdata['subtopic_content_json'] = $formdata['subtopicContentJson'];
    $formdata['quiz_content_json'] = $formdata['quizContentJson'];
    // Calculate stats for menu.
    $totaltopics = count($formdata['topics']);
    $totalsubtopics = 0;
    $totalquizzes = 0;
...
$formdata['coursesummary_helpicon'] = [  // ADD THIS SECTION.
    'text' => get_string('help_coursesummary', 'local_haccgen'),
    'component' => 'local_haccgen',
];
